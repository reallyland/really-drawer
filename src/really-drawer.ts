...
/**
really-drawer is a navigation drawer that can slide in from the left or right.
...
```html
<really-drawer opened></really-drawer>
```

Align the drawer at the end:

```html
<really-drawer align="end" opened></really-drawer>
```
...
---------------------------------|----------------------------------------|--------------------
`--really-drawer-width`             | Width of the drawer                    | 256px
`--really-drawer-content-container` | Mixin for the drawer content container | {}
`--really-drawer-scrim-background`  | Background for the scrim               | rgba(0, 0, 0, 0.5)

**NOTE:** If you use `<really-drawer>` with `<really-drawer-layout>` and specify a
value for
`--really-drawer-width`, that value must be accessible by both elements. This can
be done by defining the value on the `:host` that contains `<really-drawer-layout>`
(or `html` if outside a shadow root):

```css
:host {
  --really-drawer-width: 300px;
}
```

@group Really Elements
@element really-drawer
@demo really-drawer/demo/left-drawer.html Simple Left Drawer
@demo really-drawer/demo/right-drawer.html Right Drawer with Icons
*/
@customElement(ReallyDrawer.is)
export class ReallyDrawer extends LitElement {
  static get is() {
    return 'really-drawer';
  }
  ...
  private _translateOffset: number = 0;
  private _trackDetails = null;
  private _drawerState: number = 0;
  private _boundEscKeydownHandler = null;
  private _firstTabStop = null;
  private _lastTabStop = null;
  ...
  static get styles() {
    return [
      ...
      :host([persistent]) {
        width: var(--really-drawer-width, 256px);
      }
      ...
      #contentContainer {
        ...
        /* @apply --really-drawer-content-container; */
      }
      ...
      #scrim {
        ...
        opacity: 0;
        background: var(--really-drawer-scrim-background, rgba(0, 0, 0, 0.5));
      }
...
//       // Prevent any side effects if really-drawer closes.
...
//         this.fire('really-drawer-transitioned');
//       }
//     }
//   },
...
//     // Focus on really-drawer if it has non-zero tabindex. Otherwise, focus the
...
//     // fire when really-drawer is display: none.
//     this.debounce(
//         '_resetDrawerState', this._resetDrawerState, this.transitionDuration);
//   },
...
//   /**
//    * Fired when the layout of really-drawer has changed.
//    *
//    * @event app-reset-layout
//    */

//   /**
//    * Fired when really-drawer has finished transitioning.
//    *
//    * @event really-drawer-transitioned
//    */
// });
